...
- **La UI NO es fuente de verdad de estado (regla dura, 2026-07-18):** el estado "confirmado/aprobado" de un renglón se pinta SOLO desde el campo real de Odoo **releído del server en cada carga** — nunca desde localStorage, ni desde una marca local, ni asumiendo éxito por un HTTP 200. Tras una escritura (ej. "Enviar confirmación"), el panel debe **releer de Odoo** y pintar desde ahí; el 200 no es prueba de que el estado quedó (anti-patrón #15). localStorage guarda únicamente marcas-pendientes-de-envío, con visual **inconfundible** del confirmado (ej. "revisado ☑" gris vs "CONFIRMADO ✓" verde con fuente server). Caso 2026-07-18: en el susto del deploy se creyó que la tolerancia "sobreescribió con vacío" 31 confirmaciones del mié/jue; el forense MCP probó lo contrario — las 60 estaban `manager_approval=true` intactas desde el 16-jul, **cero writes durante/después del deploy**; lo que mentía era una **pantalla del panel no recargada** (estado en memoria viejo). Lección complementaria: una tolerancia de transición debe ser **segura para escritura** — aquí un confirm sin `origen` seguía significando "confirmar" (semántica válida, escribió `true` correcto), no hubo corrupción; el riesgo real de una tolerancia es solo si el payload viejo ESCRIBE algo con semántica distinta. Antes de "restaurar" datos por un reporte de la UI, **verificar el estado real en el server** (incl. `write_date` para detectar si de verdad hubo escritura) — no actuar sobre lo que la UI cree.
