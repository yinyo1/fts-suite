...
**Frente B — control de gastos + cierre de budget por stage (CERO-CÓDIGO):** diseño final en [`docs/finanzas/FRENTE_B_BLOQUEO_GASTOS.md`](docs/finanzas/FRENTE_B_BLOQUEO_GASTOS.md). **Decisión: descartada la Automation Rule Python → 100% cero-código.** 3 fases: **Fase 1** (stages [1,2,5,3]) gastos libres sin aviso; **Fase 2** (stages [7,9,10,13,8,4]) workflow n8n `fin/detect-gasto-cierre` (polling ~20-30min) que DETECTA facturas de proveedor (`in_invoice`/`in_refund`) posteadas sobre proyectos en cierre → correo a Esteban (no bloquea, maneja rezagos); **Fase 3** workflow n8n `project/archive-budget-cierre` (diario) que ARCHIVA la cuenta (`active=False` = prevención dura nativa, el widget filtra active=True) solo si stage-cierre **Y AR=0 Y AP=0** + log note. Distinción gasto/ingreso/cobranza por `move_type` (costos `in_invoice`; ingresos `out_invoice`; pagos/cobranza `entry` IVA-flujo). Operador `analytic_distribution 'in' [lista]` verificado (acepta listas) para detectar gastos en cierre + AP=0. Límite honesto del archivado: API/import saltan el filtro `active` del widget (solo bloquea captura manual UI). **base.automation trigger síncrono = `on_state_set`/`on_write`; diferidos `on_time*` NO bloquean.** **Fuga analítica del 99%** (gastos con `analytic_distribution` separada `{"3034":100,"1176":100}` no descuentan del budget 2-ejes; debe ser compuesta `{"3034,1176":100}`) = problema aparte de captura.
