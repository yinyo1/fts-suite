...
- **Contexto histórico (Esteban, 2026-07-09) — NO eran duplicados, era diseño:** "Analytic 1" (`GUbBF`) = **proyecto principal del día**; "Analytic 2" (`analytic_2`, hoy el alias readonly) = **segundo proyecto** cuando alguien **partía la jornada** (se editaba a mano). Se **abandonó ene-2026**; hoy se prefiere **1 solo proyecto por día** por practicidad. En los datos aparecen "iguales" porque casi nadie usó el segundo campo. ⭐ **Patrón para cuando resurja la jornada partida:** se resuelve con **DOS attendances (checkout + nuevo check-in)**, cada uno con su proyecto/bolsa — **NO** reviviendo un segundo campo analítico. Un attendance = un destino analítico.
- **✅ REUSAR** el campo Cuenta Indirecta — pero ⚠️ **CORRECCIÓN (2026-07-09, censo `DEUDA_STUDIO.md`):** `x_studio_analytic_2` (label ya "Cuenta Indirecta") es **`store=false`, `related=x_studio_many2one_field_GUbBF`, `readonly=true`** → es solo un **alias de lectura**. El **campo escribible real es `x_studio_many2one_field_GUbBF`** ("Analytic 1", `store=true`, writable). ⇒ **las ESCRITURAS (checkin, panel Confirmar Horas, Fase 3) van a `x_studio_many2one_field_GUbBF`**; las LECTURAS pueden usar cualquiera (analytic_2 lo espeja). **Cero campos nuevos.** **Recomendado para Esteban:** poner el label "Cuenta Indirecta" en **GUbBF** (el store) y dejar `analytic_2` como alias o archivarlo — así el campo editable en la UI también se llama "Cuenta Indirecta". Escribir a `analytic_2` (readonly) fallaría en silencio (anti-patrón #15). Los 2,381 valores viejos viven en GUbBF (mix con algún proyecto); no molestan (nadie los lee; escrituras nuevas serán indirecto-only).
...
**Aplicado (marcador = PAR AND, corregido tras re-canary):** el nodo `Odoo - correcciones` exige **AMBOS** marcadores en AND: `body like 'Correccion de atribucion'` **Y** `body like 'panel Confirmar Horas'`. Verificado en workflow `RBxoREDTfehELmyr` (versionId `79c2aafb`). **Por qué el par y no uno solo** (aprendido en el re-canary W3:29 W4:10):
- Solo `'Correccion de atribucion'` → dejaría pasar writes de sistema (one-shots) que copien ese texto.
- Solo `'panel Confirmar Horas'` → **demasiado ancho**: el panel escribe `(panel Confirmar Horas)` **también en las confirmaciones normales** ("Horas confirmadas por…"). El re-canary con este único marcador trajo **4 correcciones reales + 25 confirmaciones de rutina** en W3, y W4 contó confirmaciones como re-modificaciones.
- **El par AND** exige "corrección (`Correccion de atribucion`) hecha desde el panel (`panel Confirmar Horas`)" → excluye confirmaciones (no llevan `Correccion de atribucion`) **y** writes de sistema (no llevan `panel Confirmar Horas`).

Motivo raíz del autor: **TODAS** las notas tienen `author_id = 3` (Esteban) sin importar quién corrigió — `corregir-bolsa` fija el autor (regla §9/§18); el "felipe perez"/"FTS Master" vive solo en el **texto** del body. ⇒ el `author_id` NO sirve para distinguir humano vs sistema; los marcadores de texto son la única señal fiable.
...
2. **⚠️ Acoplamiento panel↔watchdog (invariante frágil, ceguera silenciosa):** el filtro del watchdog depende de los **dos textos literales `Correccion de atribucion` + `panel Confirmar Horas`** que `corregir-bolsa` (`O61Abp4s26yYpFEq`) escribe en el body. **Si se cambia cualquiera de esos textos en el panel/workflow, hay que actualizar el filtro del watchdog en el MISMO cambio**, o W3/W4 se quedan en **0 sin lanzar error** (dejan de ver correcciones y nadie se entera). Par acoplado a vigilar juntos: `corregir-bolsa` = **escritor** de los marcadores ↔ `ops/watchdog-mo` nodo `Odoo - correcciones` = **lector**. Cualquier futuro escritor de correcciones "humanas" que quiera aparecer en W3/W4 debe incluir **ambos** marcadores.

3. **Backlog (decisión pendiente Esteban):** las confirmaciones que además reflejan un cambio de atribución (ej. body "Horas confirmadas … — SO corregida a X") hoy quedan **invisibles** para W3 con el filtro AND (no llevan `Correccion de atribucion`). Decidir si deben contar como corrección en W3 o está bien que solo cuenten las correcciones explícitas del panel.
