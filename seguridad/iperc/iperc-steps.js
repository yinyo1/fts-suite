      ...
      <div style="padding:16px 18px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="border:1.5px solid #d1d5db;border-radius:10px;overflow:hidden">
          <div style="background:#f9fafb;padding:8px 12px;border-bottom:1px solid #d1d5db;display:flex;align-items:center;gap:8px">
            <span style="font-size:15px">🟩</span>
            <div style="flex:1"><strong style="font-size:12px">OpenRouter · Fallback LLaMA 70B</strong> <span style="font-size:10px;color:#666">Respaldo si Groq falla · Gratis</span></div>
            <span style="font-size:10px;font-weight:700;color:${localStorage.getItem('fts_openrouter_key')?'#16a34a':'#6b7280'}">${localStorage.getItem('fts_openrouter_key')?'✅ Activa':'⬚ Opcional'}</span>
          </div>
          <div style="padding:10px 12px">
            <div style="font-size:10px;color:#555;margin-bottom:5px">Obtén tu key en <a href="https://openrouter.ai/keys" target="_blank" style="color:#1d4ed8">openrouter.ai/keys</a> · Empieza con <strong>sk-or-...</strong></div>
            <input id="orkey-input" type="text" value="${localStorage.getItem('fts_openrouter_key')||''}" placeholder="sk-or-..." style="width:100%;padding:8px 10px;border:1.5px solid #e0e0e0;border-radius:7px;font-size:12px;font-family:Inter,sans-serif;box-sizing:border-box;margin-bottom:6px">
            <div style="display:flex;gap:6px">
...
function saveOpenRouterKey(closeModal){
  var key=(document.getElementById('orkey-input')?.value||'').trim();
...
async function testOpenRouterKey(){
  var key=(document.getElementById('orkey-input')?.value||'').trim();
  var result=document.getElementById('orkey-test-result');
  if(!key){ result.style.cssText='display:block;background:#fef2f2;border:1px solid #fca5a5;color:#dc2626;padding:6px 8px;border-radius:6px'; result.textContent='⚠️ Ingresa una key primero.'; return; }
  result.style.cssText='display:block;background:#f8f9fa;border:1px solid #e0e0e0;color:#666;padding:6px 8px;border-radius:6px';
  ...
  try{
    var res=await fetch('https://openrouter.ai/api/v1/chat/completions',{
      ...
      body:JSON.stringify({model:'meta-llama/llama-3.3-70b-instruct:free',max_tokens:5,messages:[{role:'user',content:'Di solo: OK'}]})
    });
    var d=await res.json();
    if(d.error){ result.style.cssText='display:block;background:#fef2f2;border:1px solid #fca5a5;color:#dc2626;padding:6px 8px;border-radius:6px'; result.textContent='❌ '+(d.error.message||'Key inválida'); return; }
    var txt=(d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content)||'';
    result.style.cssText='display:block;background:#f0fdf4;border:1px solid #86efac;color:#16a34a;padding:6px 8px;border-radius:6px;font-weight:500';
    result.innerHTML='✅ OpenRouter OK · <strong>llama-3.3-70b:free</strong><br><span style="font-size:10px;color:#059669">Respondió: "'+txt.trim()+'"</span>';
  }catch(e){ result.style.cssText='display:block;background:#fef2f2;border:1px solid #fca5a5;color:#dc2626;padding:6px 8px;border-radius:6px'; result.textContent='❌ Error de red: '+e.message; }
}
...
function saveAllKeys(){
  ...
  if(orKey){ localStorage.setItem('fts_openrouter_key',orKey); }
